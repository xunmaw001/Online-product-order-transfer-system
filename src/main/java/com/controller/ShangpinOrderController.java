...
public class ShangpinOrderController {
    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request) {
        ...
        if (shangpinOrder != null) {
            ...
            if (shangpin != null) {
                BeanUtils.copyProperties(shangpin, view, new String[]{"id", "createTime", "insertTime", "updateTime", "yonghuId"
                        , "shangjiaId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            ...
            //级联表 商家
            //级联表
            ShangjiaEntity shangjia = shangjiaService.selectById(shangpinOrder.getShangjiaId());
            if (shangjia != null) {
    ...
    public R save(@RequestBody ShangpinOrderEntity shangpinOrder, HttpServletRequest request) {
        ...
        if (false)
            ...
            shangpinOrder.setShangjiaId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
...
//            shangpinOrder.setShangjiaId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
    ...
    public R update1(@RequestBody ShangpinOrderEntity shangpinOrder, HttpServletRequest request) throws NoSuchFieldException, ClassNotFoundException, IllegalAccessException, InstantiationException {
        logger.debug("update方法:,,Controller:{},,shangpinOrder:{}", this.getClass().getName(), shangpinOrder.toString());
        ShangpinOrderEntity oldShangpinOrderEntity = shangpinOrderService.selectById(shangpinOrder.getId());//查询原先数据
        oldShangpinOrderEntity.setShangjiaId(null);
        shangpinOrderService.updateAllColumnById(oldShangpinOrderEntity);//根据id更新
        return R.ok();
    }
...
//                            shangpinOrderEntity.setShangjiaId(Integer.valueOf(data.get(0)));   //商家 要改的
...
//                            shangpinOrderEntity.setCreateTime(date);//时间
    ...
    public R receiving(Integer id, HttpServletRequest request) {
        ...
        ShangjiaEntity shangjiaEntity = shangjiaService.selectById(shangpinOrderEntity.getShangjiaId());
        shangjiaEntity.setNewMoney(shangjiaEntity.getNewMoney() + shangpinOrderEntity.getShangpinOrderTruePrice());
