		...
		<div v-if="showFlag">
				...
				<el-row class="ad"
					...
					<el-form-item>
						...
						<a style="text-decoration:none" class="el-button el-button--success"
							...
							href="http://localhost:8080/wangshangshangpindingdanzhuanshou/upload/shangpinOrderMuBan.xls">批量导入商品订单数据模板</a>
						&nbsp;
			...
			<div class="table-content">
				<el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
					...
					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="shangjiaName"
						header-align="center" label="接单商家">
						<template slot-scope="scope">
							{{scope.row.shangjiaName?scope.row.shangjiaName:'未结单'}}
						</template>
					</el-table-column>
					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
...
&& sessionTable=='shangjia' && scope.row.shangjiaId != null" type="primary" icon="el-icon-reading" size="mini"
								@click="openDeliver(scope.row.id)">发货</el-button>
							<el-button
								v-if="isAuth('shangpinOrder','接单') && scope.row.shangpinOrderTypes == 101 && scope.row.shangjiaId == null"
								type="primary" icon="el-icon-reading" size="mini"
								@click="jiedan(scope.row.id)">接单</el-button>
							<el-button
								v-if="isAuth('shangpinOrder','接单') && scope.row.shangpinOrderTypes == 101 && scope.row.shangjiaId != null"
								type="primary" icon="el-icon-reading" size="mini" @click="zhuandan(scope.row.id)">转单
							</el-button>
	...
	export default {
		...
		methods: {
			...
			zhuandan(id) {
				...
				this.$confirm(`确定要放弃该订单吗?`, "提示", {
					confirmButtonText: "确定",
					cancelButtonText: "取消",
					type: "warning"
				}).then(() => {
					this.$http({
						url: "shangpinOrder/update1",
						method: "post",
						data: data
					}).then(({
						data
			...
			jiedan(id) {
				let data = {
					id: id,
					shangjiaId: this.$storage.get("userId")
				}
				this.$confirm(`确定要接该订单吗?`, "提示", {
					confirmButtonText: "确定",
					cancelButtonText: "取消",
					type: "warning"
				}).then(() => {
